...
export default function BuildingDetailClient({ building: initialBuilding }: { building: Building }) {
  ...
  const handleReserve = async (index: number) => {
    ...
    if (apt.quantity > 0) {
      ...
      try {
        const res = await fetch(`http://localhost:4000/buildings/${building.id}`, {
          method: 'PUT',
